...
`cool-bible-tutor` v2.4.5 is a self-contained Codex plugin directory or archive. Install the whole directory so `.codex-plugin/plugin.json`, `assets/`, `skills/`, `scripts/`, and `tests/` remain together.
...
The marketplace package includes the manifest-bound 9,942-chunk, 1,024-dimensional `assets/rag/cuv-rag-index.sqlite3`, the pinned MIT `rag_subsystem` wheel, dependency lock, model download manifest, and launcher. It does not include model weights or a writable vector store.

After explicit consent, `python scripts/cool_bible_tutor.py setup-rag` installs the CPU-only hash-locked runtime and downloads the exact pinned model revision into per-user application data. The operation verifies every digest, loads the model offline, runs a Traditional Chinese semantic smoke query, and activates `config.json` last. Topic queries remain local. Network contacts are limited to the package indexes and model URLs declared in the release manifests.
...
```text
python -B scripts/build_marketplace_release.py --source . --destination <marketplace-root> --version 2.4.5
```
